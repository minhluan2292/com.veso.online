...
public class MessagingAnalytics {
    ...
    public static boolean shouldUploadScionMetrics(Bundle bundle) {
        ...
        return XosoConfig.REGION_ID_SOUTH.equals(bundle.getString(Constants.AnalyticsKeys.ENABLED));
    }
    ...
    private static void setUserPropertyIfRequired(Bundle bundle) {
        if (bundle != null) {
            if (XosoConfig.REGION_ID_SOUTH.equals(bundle.getString(Constants.AnalyticsKeys.TRACK_CONVERSIONS))) {
    ...
    private static void logToFirelog(MessagingClientEvent.Event event, Intent intent, TransportFactory transportFactory) {
        ...
        if (eventToProto != null) {
            try {
                transportFactory.getTransport(Constants.FirelogAnalytics.FCM_LOG_SOURCE, MessagingClientEventExtension.class, Encoding.of("proto"), MessagingAnalytics$$ExternalSyntheticLambda0.INSTANCE).send(Event.ofData(MessagingClientEventExtension.newBuilder().setMessagingClientEvent(eventToProto).build()));
    ...
    static int getPriority(Bundle bundle) {
        ...
        if (string == null) {
            if (XosoConfig.REGION_ID_SOUTH.equals(bundle.getString(Constants.MessagePayloadKeys.PRIORITY_REDUCED_V19))) {
                return 2;
            }
